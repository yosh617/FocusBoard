...
export function SettingsPanel({ open, settings, saveState, onChange: applySettings, onUndo, onClose, onStartBackgroundEditing, fullscreenSupported, onFullscreenToggle, onResetSettings, onClearTimer, onMessage, customBackgrounds, onAddBackgrounds, onRemoveBackground, onReorderBackgrounds }: Props) {
  ...
  const resetSection = (patch: Partial<AppSettings>) => { onChange(category === "clock" ? { ...patch, dateFormat: defaultSettings.dateFormat, clockBackgroundSettings: defaultSettings.clockBackgroundSettings } : category === "background" ? { ...patch, backgroundFrames: defaultSettings.backgroundFrames } : category === "display" ? { ...patch, clockColor: defaultSettings.clockColor, timerColor: defaultSettings.timerColor, clockBackgroundSettings: defaultSettings.clockBackgroundSettings } : patch); onMessage("この項目を初期値に戻しました。"); };
  ...
  return <div className="drawer-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onClose()}><aside className="settings-drawer" role="dialog" aria-modal="true" aria-labelledby="settings-title" ref={drawerRef}>
    ...
    <div className="settings-content"><section id="settings-category-panel" className="settings-section" role="tabpanel" aria-labelledby={`settings-tab-${category}`} tabIndex={0}><div className="section-heading"><h3 id="category-title">{sectionTitle}</h3>{category !== "data" && <button className="text-button" type="button" onClick={() => { const map: Record<Exclude<Category, "data">, Partial<AppSettings>> = { display: { showClock: defaultSettings.showClock, showDate: defaultSettings.showDate, showTimer: defaultSettings.showTimer, fullscreen: defaultSettings.fullscreen, fontFamily: defaultSettings.fontFamily, colorPreset: defaultSettings.colorPreset, matchBackgroundColors: defaultSettings.matchBackgroundColors }, background: { backgroundChoice: defaultSettings.backgroundChoice, overlayOpacity: defaultSettings.overlayOpacity, backgroundScale: defaultSettings.backgroundScale, backgroundPosition: defaultSettings.backgroundPosition, slideshowIntervalSec: defaultSettings.slideshowIntervalSec }, clock: { use12Hour: defaultSettings.use12Hour, showSeconds: defaultSettings.showSeconds, clockFontSize: defaultSettings.clockFontSize, dateFontSize: defaultSettings.dateFontSize, clockColor: defaultSettings.clockColor }, timer: { timerFontSize: defaultSettings.timerFontSize, timerBackgroundOpacity: defaultSettings.timerBackgroundOpacity, timerPosition: defaultSettings.timerPosition, timerColor: defaultSettings.timerColor }, pomodoro: { workMinutes: defaultSettings.workMinutes, shortBreakMinutes: defaultSettings.shortBreakMinutes, longBreakMinutes: defaultSettings.longBreakMinutes, soundEnabled: defaultSettings.soundEnabled } }; resetSection(map[category]); }}>初期値に戻す</button>}</div>
      {category === "display" && <><Toggle id="show-clock" label="時計を表示" checked={settings.showClock} onChange={(showClock) => onChange({ showClock })} /><Toggle id="show-date" label="日付を表示" checked={settings.showDate} onChange={(showDate) => onChange({ showDate })} /><Toggle id="show-timer" label="タイマーを表示" checked={settings.showTimer} onChange={(showTimer) => onChange({ showTimer })} /><Toggle id="fullscreen" label="全画面表示" checked={settings.fullscreen} disabled={!fullscreenSupported} onChange={(fullscreen) => { void onFullscreenToggle(fullscreen); }} /><p className="settings-help">時計やタイマーを画面いっぱいに表示します。対応していないブラウザでは利用できません。</p><div className="setting-control"><span className="setting-label">フォント</span><div className="choice-grid" role="radiogroup">{fonts.map((font) => <button type="button" role="radio" aria-checked={settings.fontFamily === font.value} className={settings.fontFamily === font.value ? "is-selected" : ""} style={{ fontFamily: fontOptions[font.value] }} onClick={() => onChange({ fontFamily: font.value })} key={font.value}>{font.label}</button>)}</div></div><p className="settings-help">時計・日付の色とタイマーの色は分けて設定できます。</p><Toggle id="match-colors" label="背景に合わせて自動調整" checked={settings.matchBackgroundColors} onChange={(matchBackgroundColors) => onChange({ matchBackgroundColors })} />{settings.matchBackgroundColors && <p className="settings-help">自動調整中は、時計・日付とタイマーの手動色入力を無効にします。</p>}{!settings.matchBackgroundColors && <div className="setting-control"><span className="setting-label">カラーテーマ（両方に適用）</span><div className="choice-grid" role="radiogroup" aria-label="カラーテーマ">{(Object.entries(colorPresets) as [Exclude<ColorPreset, "custom">, typeof colorPresets.sky][]).map(([id, preset]) => <button type="button" role="radio" aria-checked={settings.colorPreset === id} className={settings.colorPreset === id ? "is-selected" : ""} onClick={() => applyTheme(id)} key={id}><i style={{ background: preset.accent }} />{preset.label}</button>)}<button type="button" role="radio" aria-checked={settings.colorPreset === "custom"} className={settings.colorPreset === "custom" ? "is-selected" : ""} onClick={() => onChange({ colorPreset: "custom" })}>カスタム</button></div></div>}</>}
      {category === "clock" && <div className="color-setting-group"><h4>時計・日付の色と位置</h4><p>背景画像ごとに時計と日付の色・位置を保存します。</p><label className="background-frame-settings__target" htmlFor="clock-background-target">設定する背景<select id="clock-background-target" value={clockTarget} onChange={(event) => setClockTarget(event.target.value as BackgroundFrameTarget)}>{frameOptions.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label><ColorSetting id="clock-color" label="時計・日付の色" value={clockBackgroundSetting.color} disabled={settings.matchBackgroundColors} onChange={(color) => { updateClockSetting({ color }); onChange({ colorPreset: "custom" }); }} />{settings.matchBackgroundColors && <small className="color-setting-group__note">背景に合わせて自動調整中です。位置や表示サイズが変わると再判定します。</small>}<Range id="clock-position-x" label="時計の左右位置" value={Math.round(clockBackgroundSetting.position.x * 100)} min={0} max={100} step={1} unit="%" initial={defaultSettings.clockDatePosition.x * 100} onChange={(value) => updateClockSetting({ position: { ...clockBackgroundSetting.position, x: value / 100 } })} /><Range id="clock-position-y" label="時計の上下位置" value={Math.round(clockBackgroundSetting.position.y * 100)} min={0} max={100} step={1} unit="%" initial={defaultSettings.clockDatePosition.y * 100} onChange={(value) => updateClockSetting({ position: { ...clockBackgroundSetting.position, y: value / 100 } })} /></div>}
      ...
      {category === "timer" && <div className="color-setting-group"><h4>タイマーのアクセント色</h4><p>タイマーの進捗や操作ボタンに使う色を設定します。</p><ColorSetting id="timer-color" label="タイマーのアクセント色" value={settings.timerColor} disabled={settings.matchBackgroundColors} onChange={(timerColor) => onChange({ timerColor, colorPreset: "custom" })} />{settings.matchBackgroundColors && <small className="color-setting-group__note">背景に合わせて自動調整中です。</small>}</div>}
