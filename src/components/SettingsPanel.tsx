...
function BackgroundSettings({ settings, frame, customBackgrounds, customSize, frameOptions, frameTarget, onFrameTargetChange, onStartBackgroundEditing, onChange, uploads, move }: BackgroundSettingsProps) {
  ...
  const sourceOptions: { value: BackgroundChoice; label: string; imageUrl?: string }[] = [
    ...
    ...customBackgrounds.filter((item) => !hiddenBackgroundIds.has(item.id)).map((item) => ({ value: `custom:${item.id}` as BackgroundChoice, label: item.name, imageUrl: item.url }))
  ];
  ...
  const setBackgroundVisibility = (id: string, visible: boolean) => {
    const hiddenIds = visible
      ...
      : [...settings.hiddenBackgroundIds, id];
    const patch: Partial<AppSettings> = { hiddenBackgroundIds: hiddenIds };
    if (!visible && settings.backgroundChoice === `custom:${id}`) patch.backgroundChoice = "slideshow";
    onChange(patch);
  };
    ...
    <section className="background-source-settings" aria-labelledby="background-source-heading">
      ...
      <div className="background-picker" role="radiogroup" aria-label="背景を選択">
        {sourceOptions.map((option) => <div className="background-option-wrap" key={option.value}>
          ...
          {option.value.startsWith("custom:") && <button type="button" className="background-option__visibility" aria-label={`${option.label}を非表示`} title="この画像を非表示" onClick={() => setBackgroundVisibility(option.value.slice("custom:".length), false)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3l18 18M10.6 10.6a2 2 0 0 0 2.8 2.8M9.9 5.2A10.8 10.8 0 0 1 12 5c5.2 0 8.8 3.3 10 7-.4 1.2-1.1 2.3-2 3.2M6.2 6.2C4.5 7.3 3.3 8.9 2 12c1.2 3.7 4.8 7 10 7 1.1 0 2.1-.2 3-.5" /></svg></button>}
        </div>)}
      </div>
      ...
      {customBackgrounds.length > 0 && <details className="background-library"><summary>追加画像を管理する</summary><p className="settings-help">{(customSize / 1024 / 1024).toFixed(1)}MB使用中。矢印で表示順を変更できます。非表示にした画像はここから再表示できます。</p><div className="background-manager" aria-label="追加した背景画像">{customBackgrounds.map((item, index) => { const visible = !hiddenBackgroundIds.has(item.id); return <article className={visible ? undefined : "background-manager__item--hidden"} key={item.id}><img src={item.url} alt="" /><span>{item.name}{!visible && <small>非表示中</small>}</span><div><button type="button" aria-label={`${item.name}を${visible ? "非表示" : "表示"}`} onClick={() => setBackgroundVisibility(item.id, !visible)}>{visible ? "非表示" : "表示"}</button><button type="button" aria-label={`${item.name}を前へ`} disabled={index === 0} onClick={() => move(index, -1)}>↑</button><button type="button" aria-label={`${item.name}を後へ`} disabled={index === customBackgrounds.length - 1} onClick={() => move(index, 1)}>↓</button></div></article>; })}</div></details>}
...
export function SettingsPanel({ open, settings, saveState, onChange: applySettings, onUndo, onClose, onStartBackgroundEditing, fullscreenSupported, onFullscreenToggle, onResetSettings, onClearTimer, onMessage, adaptivePalette, customBackgrounds, onAddBackgrounds, onReorderBackgrounds }: Props) {
  const drawerRef = useRef<HTMLElement>(null); const closeRef = useRef<HTMLButtonElement>(null); const [category, setCategory] = useState<Category>("background"); const [frameTarget, setFrameTarget] = useState<BackgroundFrameTarget>("bg1"); const [clockTarget, setClockTarget] = useState<BackgroundFrameTarget | "">("");
  ...
  const clockBackgroundSetting = clockTarget ? settings.clockBackgroundSettings[clockTarget] ?? { position: defaultSettings.clockDatePosition, color: settings.clockColor } : { position: defaultSettings.clockDatePosition, color: settings.clockColor };
