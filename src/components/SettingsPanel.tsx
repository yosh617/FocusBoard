...
type Category = "appearance" | "timer" | "data";
...
type Props = {
  ...
  onResetSettings: () => void; onClearTimer: () => void; onMessage: (message: string) => void; onStartBackgroundEditing: () => void;
...
const categories: { id: Category; label: string }[] = [
  { id: "appearance", label: "見た目" }, { id: "timer", label: "タイマー" }, { id: "data", label: "保存・リセット" }
];
...
function BackgroundSettings({ settings, frame, customBackgrounds, customSize, frameOptions, frameTarget, onFrameTargetChange, onStartBackgroundEditing, onChange, uploads, move, onRemoveBackground }: BackgroundSettingsProps) {
  ...
  return <>
    <section className="background-source-settings" aria-labelledby="background-source-heading">
      <div className="background-settings-heading"><div><h4 id="background-source-heading">背景を選ぶ・追加する</h4><p>画像をクリックすると、表示する背景と調整対象を同時に切り替えます。</p></div><span className="background-settings-count">{customBackgrounds.length}枚追加済み</span></div>
      <div className="background-picker" role="radiogroup" aria-label="背景を選択">
        {sourceOptions.map((option) => <div className="background-option-wrap" key={option.value}>
          <button type="button" role="radio" aria-checked={settings.backgroundChoice === option.value} className={`background-option${settings.backgroundChoice === option.value ? " background-option--active" : ""}`} onClick={() => option.value === "slideshow" ? onChange({ backgroundChoice: option.value }) : onFrameTargetChange(option.value)}>
    ...
    <section className="background-frame-settings" aria-labelledby="background-frame-heading">
      <div className="background-frame-settings__heading"><div><h4 id="background-frame-heading">選択中の画像を調整</h4><p>上の画像一覧で選んだ画像を、設定を閉じた後の画面上で移動・拡大できます。</p></div></div>
      ...
      <div className="settings-callout background-frame-settings__guide"><strong>画面上で調整します</strong><span>上の画像一覧で画像を選んでから設定を閉じ、ホーム画面の背景を直接ドラッグ・ピンチします。自動切替に戻すときは、画像一覧の「自動切替」を選んでください。</span></div>
...
export function SettingsPanel({ open, settings, saveState, onChange: applySettings, onUndo, onClose, onStartBackgroundEditing, fullscreenSupported, onFullscreenToggle, onResetSettings, onClearTimer, onMessage, customBackgrounds, onAddBackgrounds, onRemoveBackground, onReorderBackgrounds }: Props) {
  const drawerRef = useRef<HTMLElement>(null); const closeRef = useRef<HTMLButtonElement>(null); const [category, setCategory] = useState<Category>("appearance"); const [frameTarget, setFrameTarget] = useState<BackgroundFrameTarget>("bg1"); const [clockTarget, setClockTarget] = useState<BackgroundFrameTarget>("bg1");
  ...
  useEffect(() => { if (settings.backgroundChoice !== "slideshow") setClockTarget(settings.backgroundChoice); }, [settings.backgroundChoice]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (clockTarget.startsWith("custom:") && !customBackgrounds.some((item) => `custom:${item.id}` === clockTarget)) setClockTarget("bg1");
  ...
  const clockBackgroundSetting = settings.clockBackgroundSettings[clockTarget] ?? { position: defaultSettings.clockDatePosition, color: settings.clockColor };
  ...
  const onChange = (patch: Partial<AppSettings>) => {
    if (category === "appearance" && !("backgroundFrames" in patch) && ("backgroundScale" in patch || "backgroundPosition" in patch)) {
  ...
  return <div className="drawer-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onClose()}><aside className="settings-drawer" role="dialog" aria-modal="true" aria-labelledby="settings-title" ref={drawerRef}>
    <header className="settings-header"><div><p className="eyebrow">FOCUSBOARD</p><h2 id="settings-title">設定</h2></div><div className={`save-state save-state--${saveState}`} role="status">{saveState === "saving" ? "保存中" : saveState === "failed" ? "保存失敗" : "自動保存済み"}</div><button className="icon-button" type="button" aria-label="設定を閉じる" title="設定を閉じる" onClick={onClose} ref={closeRef}>×</button></header>
    <nav className="settings-tabs" aria-label="設定カテゴリー" role="tablist">{categories.map((item) => <button id={`settings-tab-${item.id}`} type="button" role="tab" aria-selected={category === item.id} aria-controls="settings-category-panel" className={category === item.id ? "is-active" : ""} onClick={() => setCategory(item.id)} key={item.id}>{item.label}</button>)}</nav>
    <div className="settings-content"><section id="settings-category-panel" className="settings-section" role="tabpanel" aria-labelledby={`settings-tab-${category}`} tabIndex={0}><div className="section-heading"><h3 id="category-title">{sectionTitle}</h3>{category !== "data" && <button className="text-button" type="button" onClick={() => { const map: Record<Exclude<Category, "data">, Partial<AppSettings>> = { appearance: { showClock: defaultSettings.showClock, showDate: defaultSettings.showDate, showTimer: defaultSettings.showTimer, fullscreen: defaultSettings.fullscreen, fontFamily: defaultSettings.fontFamily, colorPreset: defaultSettings.colorPreset, backgroundChoice: defaultSettings.backgroundChoice, overlayOpacity: defaultSettings.overlayOpacity, backgroundScale: defaultSettings.backgroundScale, backgroundPosition: defaultSettings.backgroundPosition, backgroundFrames: defaultSettings.backgroundFrames, slideshowIntervalSec: defaultSettings.slideshowIntervalSec, use12Hour: defaultSettings.use12Hour, showSeconds: defaultSettings.showSeconds, clockFontSize: defaultSettings.clockFontSize, dateFontSize: defaultSettings.dateFontSize, clockColor: defaultSettings.clockColor, clockDatePosition: defaultSettings.clockDatePosition, clockBackgroundSettings: defaultSettings.clockBackgroundSettings, matchClockBackgroundColors: defaultSettings.matchClockBackgroundColors, matchBackgroundColors: defaultSettings.matchBackgroundColors }, timer: { timerFontSize: defaultSettings.timerFontSize, timerBackgroundOpacity: defaultSettings.timerBackgroundOpacity, timerPosition: defaultSettings.timerPosition, timerColor: defaultSettings.timerColor, matchTimerBackgroundColors: defaultSettings.matchTimerBackgroundColors, workMinutes: defaultSettings.workMinutes, shortBreakMinutes: defaultSettings.shortBreakMinutes, longBreakMinutes: defaultSettings.longBreakMinutes, soundEnabled: defaultSettings.soundEnabled } }; resetSection(map[category]); }}>初期値に戻す</button>}</div>
      {category === "appearance" && <><Toggle id="show-clock" label="時計を表示" checked={settings.showClock} onChange={(showClock) => onChange({ showClock })} /><Toggle id="show-date" label="日付を表示" checked={settings.showDate} onChange={(showDate) => onChange({ showDate })} /><Toggle id="show-timer" label="タイマーを表示" checked={settings.showTimer} onChange={(showTimer) => onChange({ showTimer })} /><Toggle id="fullscreen" label="全画面表示" checked={settings.fullscreen} disabled={!fullscreenSupported} onChange={(fullscreen) => { void onFullscreenToggle(fullscreen); }} /><p className="settings-help">表示するものは、下の画面にすぐ反映されます。</p><div className="setting-control"><span className="setting-label">フォント</span><div className="choice-grid" role="radiogroup">{fonts.map((font) => <button type="button" role="radio" aria-checked={settings.fontFamily === font.value} className={settings.fontFamily === font.value ? "is-selected" : ""} style={{ fontFamily: fontOptions[font.value] }} onClick={() => onChange({ fontFamily: font.value })} key={font.value}>{font.label}</button>)}</div></div><Disclosure label="カラーテーマ"><p className="settings-help">時計・日付・タイマーの色をまとめて変更します。</p><div className="choice-grid" role="radiogroup" aria-label="カラーテーマ">{(Object.entries(colorPresets) as [Exclude<ColorPreset, "custom">, typeof colorPresets.sky][]).map(([id, preset]) => <button type="button" role="radio" aria-checked={settings.colorPreset === id} className={settings.colorPreset === id ? "is-selected" : ""} onClick={() => applyTheme(id)} key={id}><i style={{ background: preset.accent }} />{preset.label}</button>)}<button type="button" role="radio" aria-checked={settings.colorPreset === "custom"} className={settings.colorPreset === "custom" ? "is-selected" : ""} onClick={() => onChange({ colorPreset: "custom" })}>カスタム</button></div></Disclosure></>}
      {category === "appearance" && <BackgroundSettings settings={settings} frame={backgroundFrame} customBackgrounds={customBackgrounds} customSize={customSize} frameOptions={frameOptions} frameTarget={frameTarget} onFrameTargetChange={(target) => { setFrameTarget(target); onChange({ backgroundChoice: target }); }} onStartBackgroundEditing={onStartBackgroundEditing} onChange={onChange} uploads={uploads} move={move} onRemoveBackground={onRemoveBackground} />}
      {category === "appearance" && <Disclosure label="時計・日付の詳細設定"><div className="color-setting-group"><h4>時計・日付</h4><p>背景ごとの位置と色を調整します。</p><Toggle id="match-clock-colors" label="背景に合わせて自動調整" checked={settings.matchClockBackgroundColors} onChange={(matchClockBackgroundColors) => onChange({ matchClockBackgroundColors })} />{settings.matchClockBackgroundColors ? <small className="color-setting-group__note">背景に合わせて読みやすい色にします。</small> : <ColorSetting id="clock-color" label="時計・日付の色" value={clockBackgroundSetting.color} disabled={false} onChange={(color) => { updateClockSetting({ color }); onChange({ colorPreset: "custom" }); }} />}<Disclosure label="背景ごとの位置と色"><label className="background-frame-settings__target" htmlFor="clock-background-target">設定する背景<select id="clock-background-target" value={clockTarget} onChange={(event) => setClockTarget(event.target.value as BackgroundFrameTarget)}>{frameOptions.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label><Range id="clock-position-x" label="時計の左右位置" value={Math.round(clockBackgroundSetting.position.x * 100)} min={0} max={100} step={1} unit="%" initial={defaultSettings.clockDatePosition.x * 100} onChange={(value) => updateClockSetting({ position: { ...clockBackgroundSetting.position, x: value / 100 } })} /><Range id="clock-position-y" label="時計の上下位置" value={Math.round(clockBackgroundSetting.position.y * 100)} min={0} max={100} step={1} unit="%" initial={defaultSettings.clockDatePosition.y * 100} onChange={(value) => updateClockSetting({ position: { ...clockBackgroundSetting.position, y: value / 100 } })} /></Disclosure></div><Disclosure label="表示形式とサイズ"><Toggle id="use-12-hour" label="12時間表示" checked={settings.use12Hour} onChange={(use12Hour) => onChange({ use12Hour })} /><Toggle id="show-seconds" label="秒を表示" checked={settings.showSeconds} onChange={(showSeconds) => onChange({ showSeconds })} /><div className="setting-control"><label htmlFor="date-format-preset">日付の形式</label><select id="date-format-preset" value={dateFormatPresets.some((preset) => preset.value === settings.dateFormat) ? settings.dateFormat : "custom"} onChange={(event) => onChange({ dateFormat: event.target.value === "custom" ? customDateFormatExample : event.target.value })}><option value="custom">カスタム</option>{dateFormatPresets.map((preset) => <option value={preset.value} key={preset.value}>{preset.label}</option>)}</select>{!dateFormatPresets.some((preset) => preset.value === settings.dateFormat) && <><label className="sub-label" htmlFor="date-format">カスタム形式</label><input id="date-format" className="text-input" type="text" value={settings.dateFormat} maxLength={40} onChange={(event) => onChange({ dateFormat: event.target.value })} /><small>yyyy / yy、mm / m、dd / d、weekday（曜日）、weekdayShort（短い曜日）が使えます。</small></>}</div><Range id="clock-size" label="時計サイズ" value={settings.clockFontSize} {...settingRanges.clockFontSize} unit="" formatValue={(value) => describeFontSize(value, defaultSettings.clockFontSize, settingRanges.clockFontSize.min, settingRanges.clockFontSize.max)} rangeText="小さめから大きめまで調整できます。" initial={defaultSettings.clockFontSize} onChange={(clockFontSize) => onChange({ clockFontSize })} /><Range id="date-size" label="日付サイズ" value={settings.dateFontSize} {...settingRanges.dateFontSize} unit="" formatValue={(value) => describeFontSize(value, defaultSettings.dateFontSize, settingRanges.dateFontSize.min, settingRanges.dateFontSize.max)} rangeText="小さめから大きめまで調整できます。" initial={defaultSettings.dateFontSize} onChange={(dateFontSize) => onChange({ dateFontSize })} /><p className="settings-help">時計と日付は画面上でドラッグして移動できます。</p></Disclosure></Disclosure>}
      {category === "timer" && <><div className="color-setting-group"><h4>タイマー</h4><p>進捗表示と操作ボタンの色を設定します。</p><Toggle id="match-timer-colors" label="背景に合わせて自動調整" checked={settings.matchTimerBackgroundColors} onChange={(matchTimerBackgroundColors) => onChange({ matchTimerBackgroundColors })} />{settings.matchTimerBackgroundColors ? <small className="color-setting-group__note">背景の明るさと色から、タイマーを見やすく調整します。</small> : <ColorSetting id="timer-color" label="タイマーのアクセント色" value={settings.timerColor} disabled={false} onChange={(timerColor) => onChange({ timerColor, colorPreset: "custom" })} />}</div><Disclosure label="タイマーの表示と配置"><Range id="timer-size" label="タイマーサイズ" value={settings.timerFontSize} {...settingRanges.timerFontSize} unit="" formatValue={(value) => describeFontSize(value, defaultSettings.timerFontSize, settingRanges.timerFontSize.min, settingRanges.timerFontSize.max)} rangeText="小さめから大きめまで調整できます。" initial={defaultSettings.timerFontSize} onChange={(timerFontSize) => onChange({ timerFontSize })} /><Range id="timer-opacity" label="タイマー背景の不透明度" value={Math.round(settings.timerBackgroundOpacity * 100)} {...settingRanges.timerBackgroundOpacity} initial={Math.round(defaultSettings.timerBackgroundOpacity * 100)} onChange={(value) => onChange({ timerBackgroundOpacity: value / 100 })} /><PositionGrid label="開始前タイマーの配置" value={settings.timerPosition} onChange={(timerPosition) => onChange({ timerPosition })} /></Disclosure></>}
