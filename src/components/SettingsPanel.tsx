...
const uiAccentSavedColors: ColorPickerSavedColor[] = uiAccentPresets.map(({ label, color }) => ({ label, color }));
const timerSavedColors: ColorPickerSavedColor[] = uiAccentSavedColors;
...
function Disclosure({ label, children }: { label: string; children: ReactNode }) {
  return <details className="settings-disclosure" style={{ overflow: "visible" }}><summary><span>{label}</span><span className="settings-disclosure__chevron" aria-hidden="true" /></summary><div className="settings-disclosure__body">{children}</div></details>;
}
...
function ClockColorChoices({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <div className="clock-color-settings">
    <ColorPicker
      ...
      label="時計・日付の色"
      value={value}
      themeColors={clockSavedColors}
      onChange={onChange}
    />
  </div>;
}
function ThemeColorChoices({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <div className="theme-color-settings">
    <ColorPicker id="ui-accent-color" label="アプリのテーマ" value={value} themeColors={uiAccentSavedColors} onChange={onChange} />
  </div>;
}
...
export function SettingsPanel({ open, settings, orientation, saveState, onChange: applySettings, onUndo, onClose, onOpenTasks, onStartBackgroundEditing, fullscreenSupported, onFullscreenToggle, onResetSettings, onClearTimer, onMessage, adaptivePalette, customBackgrounds, onAddBackgrounds, onRemoveBackground, onReorderBackgrounds }: Props) {
  ...
  const resetPatches: Record<ResettableCategory, Partial<AppSettings>> = {
    ...
    display: {
      ...
      dateDisplayStyle: defaultSettings.dateDisplayStyle,
    ...
    timer: {
      ...
      pomodoroEndBehavior: defaultSettings.pomodoroEndBehavior,
      timerNotificationBehavior: defaultSettings.timerNotificationBehavior
    }
  };
  ...
  const saveStateLabel = saveState === "saving" ? "保存中" : saveState === "failed" ? "保存失敗" : "保存済み";

  return <div className="drawer-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onClose()}><aside className="settings-drawer" role="dialog" aria-modal="true" aria-labelledby="settings-title" aria-describedby="settings-save-state" ref={drawerRef}>
    <span className="settings-sheet-handle" aria-hidden="true" />
    <header className="settings-header">
      <div className="settings-header__identity"><h2 id="settings-title">設定</h2></div>
      <div id="settings-save-state" className={`save-state save-state--${saveState}`} role="status" aria-live="polite"><span className="save-state__label">{saveStateLabel}</span></div>
      <div className="settings-header__actions" aria-label="設定の操作">
        <button className="panel-switch-button" type="button" aria-label="タスクを開く" onClick={() => onOpenTasks()}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 6h10M9 12h10M9 18h10M4 6h.01M4 12h.01M4 18h.01" /></svg><span>タスク</span></button>
    ...
    <div className={`settings-content settings-content--${category}`} ref={contentRef}>
      ...
      {category === "display" && <><Toggle id="show-clock" label="時計を表示" checked={settings.showClock} onChange={(showClock) => onChange({ showClock })} /><Toggle id="show-date" label="日付を表示" checked={settings.showDate} onChange={(showDate) => onChange({ showDate })} /><div className="setting-control"><span id="date-display-style-label" className="setting-label">日付の表示</span><div className="choice-grid" role="radiogroup" aria-labelledby="date-display-style-label"><button type="button" role="radio" aria-checked={settings.dateDisplayStyle === "text"} className={settings.dateDisplayStyle === "text" ? "is-selected" : ""} onClick={() => onChange({ dateDisplayStyle: "text" })}>標準</button><button type="button" role="radio" aria-checked={settings.dateDisplayStyle === "calendar"} className={settings.dateDisplayStyle === "calendar" ? "is-selected" : ""} onClick={() => onChange({ dateDisplayStyle: "calendar" })}>カレンダー</button></div></div><Toggle id="fullscreen" label="全画面表示" checked={settings.fullscreen} disabled={!fullscreenSupported} onChange={(fullscreen) => { void onFullscreenToggle(fullscreen); }} /><Disclosure label="テーマの色"><div className="color-setting-group color-setting-group--disclosure"><p>選択状態と主要操作に使う色を設定します。</p><ThemeColorChoices value={settings.uiAccentColor} onChange={(uiAccentColor) => onChange({ uiAccentColor })} /></div></Disclosure><Disclosure label="タスク画面"><section className="setting-control task-theme-setting" aria-labelledby="task-theme-label"><span id="task-theme-label" className="setting-label">テーマ</span><div className="task-theme-grid" role="radiogroup" aria-labelledby="task-theme-label">{(Object.entries(taskThemePresets) as [TaskThemePreset, typeof taskThemePresets.coral][]).map(([id, preset]) => <button type="button" role="radio" aria-checked={settings.taskTheme === id} className={settings.taskTheme === id ? "is-selected" : ""} onClick={() => onChange({ taskTheme: id })} key={id}><span className="task-theme-option__swatch" style={{ background: preset.primary, color: preset.text }}>Aa</span><span><strong>{preset.label}</strong></span><span className="task-theme-option__preview" style={{ background: preset.primary, color: preset.text }}>開始</span></button>)}</div></section><div className="setting-control"><span id="task-card-visibility-label" className="setting-label">メイン画面のタスクカード</span><div className="choice-grid" role="radiogroup" aria-labelledby="task-card-visibility-label" aria-describedby="task-card-visibility-description"><button type="button" role="radio" aria-checked={settings.taskLauncherVisibility === "always"} className={settings.taskLauncherVisibility === "always" ? "is-selected" : ""} onClick={() => onChange({ taskLauncherVisibility: "always" })}>常に表示</button><button type="button" role="radio" aria-checked={settings.taskLauncherVisibility === "background-tap"} className={settings.taskLauncherVisibility === "background-tap" ? "is-selected" : ""} onClick={() => onChange({ taskLauncherVisibility: "background-tap" })}>背景タップ時のみ</button></div><small id="task-card-visibility-description">背景タップ後に詳細カードを数秒表示します。タスクボタンは常に使えます。</small></div></Disclosure></>}
      ...
      {category === "timer" && <><Toggle id="show-timer" label="タイマーを表示" checked={settings.showTimer} onChange={(showTimer) => onChange({ showTimer })} /><div className="color-setting-group"><h4>タイマー</h4><p>進捗表示と操作ボタンの色を設定します。</p><Toggle id="match-timer-colors" label="背景に合わせて自動調整" checked={settings.matchTimerBackgroundColors} onChange={(matchTimerBackgroundColors) => onChange({ matchTimerBackgroundColors })} />{settings.matchTimerBackgroundColors ? <small className="color-setting-group__note">背景の明るさと色から、タイマーを見やすく調整します。</small> : <Disclosure label="タイマーの色"><ColorPicker id="timer-color" label="タイマーの色" value={settings.timerColor} themeColors={timerSavedColors} onChange={(timerColor) => onChange({ timerColor, colorPreset: "custom" })} /></Disclosure>}</div><Disclosure label="タイマーの表示と配置"><Range id="timer-size" label="タイマーサイズ" value={settings.timerFontSize} {...settingRanges.timerFontSize} unit="" formatValue={(value) => describeFontSize(value, defaultSettings.timerFontSize, settingRanges.timerFontSize.min, settingRanges.timerFontSize.max)} initial={defaultSettings.timerFontSize} onChange={(timerFontSize) => onChange({ timerFontSize })} /><Range id="timer-opacity" label="タイマー背景の不透明度" value={Math.round(settings.timerBackgroundOpacity * 100)} {...settingRanges.timerBackgroundOpacity} initial={Math.round(defaultSettings.timerBackgroundOpacity * 100)} onChange={(value) => onChange({ timerBackgroundOpacity: value / 100 })} /><div className="orientation-picker"><span className="setting-label">設定する画面の向き</span><div role="radiogroup" aria-label="タイマーを設定する画面の向き">{orientations.map((item) => <button type="button" role="radio" aria-checked={positionOrientation === item} className={positionOrientation === item ? "is-selected" : ""} onClick={() => setPositionOrientation(item)} key={item}>{orientationLabels[item]}</button>)}</div><small>現在は{orientationLabels[orientation]}です。</small></div><PositionGrid label="開始前タイマーの配置" value={settings.timerPositions[positionOrientation]} onChange={(timerPosition) => onChange({ timerPosition, timerPositions: { ...settings.timerPositions, [positionOrientation]: timerPosition } })} /></Disclosure></>}
      {category === "timer" && <Disclosure label="ポモドーロの詳細設定"><Range id="work" label="作業時間" value={settings.workMinutes} {...settingRanges.workMinutes} initial={defaultSettings.workMinutes} onChange={(workMinutes) => onChange({ workMinutes })} /><Range id="short-break" label="短い休憩" value={settings.shortBreakMinutes} {...settingRanges.shortBreakMinutes} initial={defaultSettings.shortBreakMinutes} onChange={(shortBreakMinutes) => onChange({ shortBreakMinutes })} /><Range id="long-break" label="長い休憩" value={settings.longBreakMinutes} {...settingRanges.longBreakMinutes} initial={defaultSettings.longBreakMinutes} onChange={(longBreakMinutes) => onChange({ longBreakMinutes })} /><div className="setting-control"><span id="pomodoro-end-behavior-label" className="setting-label">ポモドーロ終了時</span><div className="choice-grid" role="radiogroup" aria-labelledby="pomodoro-end-behavior-label" aria-describedby="pomodoro-end-behavior-description"><button type="button" role="radio" aria-checked={settings.pomodoroEndBehavior === "next"} className={settings.pomodoroEndBehavior === "next" ? "is-selected" : ""} onClick={() => onChange({ pomodoroEndBehavior: "next" })}>次のセッションを準備</button><button type="button" role="radio" aria-checked={settings.pomodoroEndBehavior === "overtime"} className={settings.pomodoroEndBehavior === "overtime" ? "is-selected" : ""} onClick={() => onChange({ pomodoroEndBehavior: "overtime" })}>止めるまで延長</button></div><small id="pomodoro-end-behavior-description">「止めるまで延長」を選ぶと、終了後も延長中のまま進みます。タイマーの「終了」を押すまで止まりません。</small></div><div className="setting-control setting-control--three-choice"><span id="timer-notification-behavior-label" className="setting-label">システム通知のタイミング</span><div className="choice-grid" role="radiogroup" aria-labelledby="timer-notification-behavior-label" aria-describedby="timer-notification-behavior-description"><button type="button" role="radio" aria-checked={settings.timerNotificationBehavior === "always"} className={settings.timerNotificationBehavior === "always" ? "is-selected" : ""} onClick={() => onChange({ timerNotificationBehavior: "always" })}>常に</button><button type="button" role="radio" aria-checked={settings.timerNotificationBehavior === "background"} className={settings.timerNotificationBehavior === "background" ? "is-selected" : ""} onClick={() => onChange({ timerNotificationBehavior: "background" })}>バックグラウンドのみ</button><button type="button" role="radio" aria-checked={settings.timerNotificationBehavior === "off"} className={settings.timerNotificationBehavior === "off" ? "is-selected" : ""} onClick={() => onChange({ timerNotificationBehavior: "off" })}>通知しない</button></div><small id="timer-notification-behavior-description">ブラウザの通知を許可している場合に適用されます。アプリ内のお知らせは常に表示します。</small></div><Toggle id="sound" label="終了音" checked={settings.soundEnabled} onChange={(soundEnabled) => onChange({ soundEnabled })} /></Disclosure>}
      {category === "data" && <>
