...
type Category = "display" | "background" | "clock" | "timer" | "pomodoro" | "data";
...
const categories: { id: Category; label: string }[] = [
  ...
  { id: "timer", label: "タイマー" }, { id: "pomodoro", label: "ポモドーロ" }, { id: "data", label: "データ管理" }
];
...
export function SettingsPanel({ open, settings, saveState, onChange: applySettings, onUndo, onClose, onStartBackgroundEditing, fullscreenSupported, onFullscreenToggle, onResetSettings, onClearTimer, onMessage, customBackgrounds, onAddBackgrounds, onRemoveBackground, onReorderBackgrounds }: Props) {
  const drawerRef = useRef<HTMLElement>(null); const closeRef = useRef<HTMLButtonElement>(null); const [category, setCategory] = useState<Category>("display"); const [frameTarget, setFrameTarget] = useState<BackgroundFrameTarget>("bg1"); const [clockTarget, setClockTarget] = useState<BackgroundFrameTarget>("bg1");
  ...
  const resetSection = (patch: Partial<AppSettings>) => { onChange(category === "clock" ? { ...patch, dateFormat: defaultSettings.dateFormat, clockBackgroundSettings: defaultSettings.clockBackgroundSettings, matchClockBackgroundColors: defaultSettings.matchClockBackgroundColors } : category === "background" ? { ...patch, backgroundFrames: defaultSettings.backgroundFrames } : category === "display" ? { ...patch, clockColor: defaultSettings.clockColor, timerColor: defaultSettings.timerColor, clockBackgroundSettings: defaultSettings.clockBackgroundSettings, matchClockBackgroundColors: defaultSettings.matchClockBackgroundColors, matchTimerBackgroundColors: defaultSettings.matchTimerBackgroundColors, matchBackgroundColors: defaultSettings.matchBackgroundColors } : category === "timer" ? { ...patch, matchTimerBackgroundColors: defaultSettings.matchTimerBackgroundColors } : patch); onMessage("この項目を初期値に戻しました。"); };
