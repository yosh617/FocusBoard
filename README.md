...
- 背景画像を画面上でドラッグ・ピンチ・ホイール操作できる背景トリミング設定。設定画面では画像ごとのプレビューを見ながらiPad／iPhoneの壁紙編集に近い操作が可能
...
背景画像ごとの調整は「設定 → 背景 → 画像ごとの表示」から行います。画像を選んでプレビューを指でドラッグすると位置を動かせます。2本指のピンチで拡大縮小でき、PCではホイールやキーボードも利用できます。下のスライダーは従来どおり利用できます。
