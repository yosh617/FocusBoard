...
- 時計・日付は表示位置付近の背景を解析し、読みやすいコントラストの文字色へ自動調整。タイマーのアクセント色も背景に合わせられるオプション
- 背景画像を画面上でドラッグ・ピンチ・ホイール操作できる背景トリミング設定。設定画面で画像を選び、表示画面上でiPad／iPhoneの壁紙編集に近い操作が可能
...
背景画像ごとの調整は「設定 → 背景 → 画像ごとの表示」で画像を選び、「設定を閉じて画面上で調整」を押して行います。ホーム画面の背景を指でドラッグすると移動、2本指のピンチで拡大縮小できます。PCではホイールやキーボード、折りたたみ式スライダーも利用できます。
